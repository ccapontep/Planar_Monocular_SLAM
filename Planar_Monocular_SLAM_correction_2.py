...
from Planar_Monocular_SLAM_functools import box_plus, box_minus, v2t, t2v, t2v2
...
#            print('h_all', h_all.shape)
    ...
    if (number_known_landmarks > 0):
        ...
        # In Homogenous Transformation
        mu = t2v(MU * np.linalg.inv((v2t(K[0:2,0], K[2,0]))))
        
        ran_K = int((len(K)-3)/2)+1
        for i in range(3,ran_K):
            delta_LK = np.linalg.inv(v2t(K[2*i:(2*i+2), 0], 0))
            pred_H = v2t(h_t[2*i:(2*i+2), 0], 0)
#            h_tt = v2t(h_t[2*i:2*i+1], 0)
            delta_K = pred_H * delta_LK
            pert = t2v2(delta_K)
            state_pert = np.hstack(pert)
        state_pert = state_pert - z_t
        print(state_pert)
#            delta_KK = np.hstack(delta_LK)
#        print(delta_KK)
#        print(delta_KK.shape)
#        delta_K = np.hstack([delta_MU, delta_KK])

        #update mu
#        innovation = z_t - h_t #box_minus(z_t, h_t)
#        mu         = mu + K*innovation  
#        mu         = np.linalg.inv(delta_K) * h_t - z_t
